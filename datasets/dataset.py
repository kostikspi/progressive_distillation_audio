...
from models.params import params as diff_params
...
class ConditionalDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        spectrogram = transform(audio_filename, self.params)
        return {
            'audio': signal[0],
...
class Collator:
    ...
    def collate(self, minibatch):
        ...
        audio_path = [record['audio_path'] for record in minibatch if 'audio_path' in record]
        return {
            'audio': torch.from_numpy(audio),
...
def from_path(data_dirs, is_distributed=False, batch_size=1, params_type=None):
    # print(params.batch_size)
    params = None
    if params_type == 'diffwave' or params_type is None:
        params = diff_params
        dataset = ConditionalDataset(data_dirs, params=diff_params)
    else:
        params = wavegrad_params
        dataset = ConditionalDataset(data_dirs, params=wavegrad_params)
