...
class GaussianDiffusion:
    ...
    def inference(self, x, t, extra_args):
        return self.net_(x, t * self.time_scale, spectrogram=extra_args)
...
class GaussianDiffusionDefault(GaussianDiffusion):
    ...
    def distill_loss(self, student_diffusion, x, t, extra_args, eps=None, student_device=None, device='cuda'):
        # TO DO:
        # device = 'cpu'
        if eps is None:
        ...
        with torch.no_grad():
            ...
            # print(alpha_1.shape)
            # print('Z: ', z.shape)
            v = self.inference(z.to(device).float(), t + 1, extra_args).double().squeeze(1)
            # torch.save(v, 'test.pt')
            # print('V :', v.shape)
            rec = (alpha * z - sigma * v).clip(-1, 1)
            # print('REC: ', rec.shape)
            z_1 = alpha_1 * rec + (sigma_1 / sigma) * (z - alpha * rec)
            # print('Z_1: ', z_1.shape)
            v_1 = self.inference(z_1.float(), t, extra_args).double()
        ...
        v = student_diffusion.net_(z.float(), t * self.time_scale, spectrogram=extra_args)
        my_rec = (alpha_s * z - sigma_s * v).clip(-1, 1)
        return F.mse_loss(w * v.float(), w * v_2.float()), v, v_2


class DiffwaveDiffusion(GaussianDiffusion):
    def __init__(self, net, betas, time_scale=1, gamma=0.3, params=None):
        super.__init__(net, betas, time_scale)
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.noise_schedule = np.linspace(1e-4, 0.05, args[2]["n_timesteps"]).tolist(),
        self.inference_noise_schedule = [0.0001, 0.001, 0.01, 0.05, 0.2, 0.5]
        # self.params = params
        self.gamma = 0.3
        self.beta = torch.tensor(self.noise_schedule).squeeze()
    ...
    def distill_loss(self, student_diffusion, x, t, extra_args, eps=None, student_device=None, device='cuda'):
        ...
        # predicted_1 = self.net_(noisy_audio, t, spectrogram).squeeze(1)
        alpha = (1 - self.beta).squeeze()
        alpha_cum = torch.tensor(np.cumprod(alpha.cpu())).to(device)
        inf_audio = noisy_audio
        with torch.no_grad():
            for n in range(0, 2):
                t -= n
                # print(alpha.shape)
                # print(t)
                c1 = (1 / alpha[t] ** 0.5).unsqueeze(1)
                c2 = (self.beta[t] / (1 - alpha_cum[t]) ** 0.5).unsqueeze(1)
                teacher_result = self.net_(inf_audio,
                                           torch.tensor(t, device=inf_audio.device),
                                           spectrogram).squeeze(1)
                inf_audio = c1 * (inf_audio - c2 * teacher_result)
                if n < 1:
                    noise = torch.randn_like(inf_audio)
                    sigma = ((1.0 - alpha_cum[t - 1]) / (1.0 - alpha_cum[t]) * self.beta[t]) ** 0.5
                    inf_audio += sigma.unsqueeze(1) * noise
                inf_audio = torch.clamp(inf_audio, -1.0, 1.0)
            # predicted_2 = self.net_(predicted_1, t + 1, spectrogram)

        v = student_diffusion.net_(noisy_audio.float(), t * self.time_scale, spectrogram=spectrogram).squeeze(1)

        # loss = self.loss_fn(noise, predicted.squeeze(1))
        # alpha_s, sigma_s = student_diffusion.get_alpha_sigma(x, t // 2)
        c1 = 1 / alpha[t] ** 0.5
        c2 = self.beta[t] / (1 - alpha_cum[t]) ** 0.5
        student_audio = torch.clamp(c1.unsqueeze(1) * (noisy_audio - c2.unsqueeze(1) * v), -1.0, 1.0)
        ...
        loss = F.mse_loss(w * v.float(), w * teacher_result.float())
        return loss, student_audio, inf_audio
...
class WaveGradDiffusion(GaussianDiffusion):
    ...
    def distill_loss(self, student_diffusion, x, t, extra_args, eps=None, student_device=None, device='cuda'):
        ...
        noisy_audio = noise_scale[t] * audio + (1.0 - noise_scale[t] ** 2) ** 0.5 * noise
        inf_audio = noisy_audio
        ...
        with torch.no_grad():
            for n in range(0, 2):
                ...
                teacher_result = self.net_(inf_audio,
                                           spectrogram,
                                           noise_scale[t].squeeze(1)).squeeze(1)
                inf_audio = c1 * (inf_audio - c2 * teacher_result)
                if n < 1:
                    noise = torch.randn_like(inf_audio)
                    sigma = ((1.0 - alpha_cum[t - 1]) / (1.0 - alpha_cum[t]) * self.beta[t]) ** 0.5
        ...
        v = student_diffusion.net_(noisy_audio.float(),
                                   spectrogram,
                                   self.noise_level[(t * self.time_scale).long()]).squeeze(1)
