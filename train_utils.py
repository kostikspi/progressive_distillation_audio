...
class DiffusionDistillation:
    ...
    def __init__(self, scheduler, n_timesteps=50):
        self.scheduler = scheduler
        self.noise_schedule = np.linspace(1e-4, 0.05, n_timesteps).tolist()
    ...
    def train_student(self, distill_train_loader, teacher_diffusion, student_diffusion, student_ema, student_lr,
                      device, make_extra_args=make_none_args, on_iter=default_iter_callback, log_count=100):
        scheduler = self.scheduler
        total_steps = len(distill_train_loader)
        scheduler.init(student_diffusion, student_lr, total_steps)
        teacher_diffusion.net_.eval()
        ...
        for item in pbar:
            ...
            extra_args = item['spectrogram'].to(device)
            path = item['audio_path']
            scheduler.zero_grad()
            # print(audio.shape)
            # time = 2 * torch.randint(0, student_diffusion.num_timesteps, [audio.shape[0]], device=device)
            time = torch.randint(0, len(self.noise_schedule), [audio.shape[0]], device=audio.device)
            # print(time.shape)
            # extra_args = make_extra_args(audio, label, device)
            loss, pred, target = teacher_diffusion.distill_loss(student_diffusion, audio, time, extra_args)
            L = loss.item()
            L_tot += L
            N += 1
            pbar.set_description(f"Loss: {L_tot / N}")
            ...
            if scheduler.stop(N, total_steps):
                break
            # on_iter(N, loss.item())
            logs = {'loss': loss.item(), "current_lr": 1}
            preds += pred
            targets += target
            ground_truth += audio
            target_spec += extra_args
            paths += path
            times += time
            if N % log_count == 0:
                logs['preds'] = preds
                logs['target'] = targets
                logs['ground_truth'] = ground_truth
                logs['target_spec'] = target_spec
                logs['audio_path'] = paths
                logs['d_step'] = times
                logs['step'] = N
                logs['log_audio'] = True
                on_iter(logs)
                logs['log_audio'] = False
                preds = []
                targets = []
                target_spec = []
                paths = []
